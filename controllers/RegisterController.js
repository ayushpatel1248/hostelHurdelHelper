...
const {registrationNumber , email , mobileNumber , password } = req.body;
try{
    await schema.validateAsync({registrationNumber , email , mobileNumber, password});
    ...
    if(foundUser == null){ 
        ...
        var  a = await UserService.registerUser(registrationNumber , email , mobileNumber, hash)
        const token = jwt.sign({_id:a._id},SECRET_KEY);
        let deepcopy = JSON.parse(JSON.stringify(a))
        delete deepcopy["password"] 
        res.send({
            status:"ok",
            msg: "user registration sucessfully complete",
